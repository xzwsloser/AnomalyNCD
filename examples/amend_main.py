...
from models.amend import AMEND
...
def get_args():
    parser = argparse.ArgumentParser(description='AMEND', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument('--config', type=str, default='./configs/AnomalyNCD_amend.yaml', help='config file path')
    parser.add_argument('--runner_name', default='AMEND', type=str)
...
def load_args(cfg, args):
    ...
    amend_cfg = cfg['amend']
    args.amend_neighbors = int(amend_cfg['neighbors'])
    args.amend_expanded_neighbors = int(amend_cfg['expanded_neighbors'])
    args.amend_bank_size = int(amend_cfg['bank_size'])
    args.amend_expanded_affinity = float(amend_cfg['expanded_affinity'])
    args.amend_margin_weight = float(amend_cfg['margin_weight'])
    args.amend_projection_dim = int(amend_cfg['projection_dim'])
    args.amend_temperature = float(amend_cfg.get('temperature', 0.1))
